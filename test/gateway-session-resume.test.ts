...
readline.createInterface({ input: process.stdin }).on('line', line => {
  ...
  if (!resume) {
    send({ type: 'assistant', message: { content: [{ type: 'tool_use', id: 'call_1', name: 'mcp__local_agent__read_file', input: { path: 'README.md' } }] } })
    emitStop('tool_use')
    return
  }
  send({ type: 'assistant', message: { content: [{ type: 'text', text: '<LOCAL_AGENT_FINAL>RESUMED_OK' }] } })
  emitStop('end_turn')
})
`,
  )
  ...
  assert.match(resumeInputs[0].line.message.content[0].text, /"tool_name":"read_file"/)
  assert.match(resumeInputs[0].line.message.content[0].text, /README CONTENT/)
